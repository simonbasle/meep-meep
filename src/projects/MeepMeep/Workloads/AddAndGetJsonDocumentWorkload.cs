...
namespace MeepMeep.Workloads
{
    ...
    public class AddAndGetJsonDocumentWorkload : WorkloadBase
    {
        ...
        protected override WorkloadOperationResult OnExecuteStep(IBucket client, int workloadIndex, int docIndex, Stopwatch sw)
        {
            ...
            var storeOpResult = client.Upsert(key, SampleDocument);
            var getOpResult = client.Get<string>(randomKey);
        ...
        protected virtual string GetMessage(IOperationResult<string> storeOpResult, IOperationResult<string> getOpResult)
        {
